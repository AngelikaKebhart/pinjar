...
/**
 * A modal workspace opened from the header's manage menu.
 *
  ...
  return (
    ...
    */
    <dialog
      ...
      */
      className="m-auto max-h-[calc(100vh-4rem)] w-[min(30rem,calc(100vw-2rem))] overflow-y-auto rounded-card border border-line bg-surface p-6 text-ink shadow-card backdrop:bg-black/50"
    >
      ...
      {/*
        Mounted only while open, so the contents read storage when asked rather
        than watching it for the whole life of the tab.
      */}
      {isOpen && children}
    </dialog>
  );
