...
- **Ausnahme Vorschaubilder – die einzige Netzwerkanfrage der Extension:** Gespeichert wird nur die *URL* des Bildes, nicht das Bild selbst (Begründung: Speicherplatz, siehe unten). Beim Öffnen des Dashboards lädt der Browser diese Bilder folglich von den ursprünglichen Servern nach. Der jeweilige Shop erfährt dadurch IP-Adresse und Zeitpunkt – also dass jemand gerade seine Merkliste ansieht. Es werden dabei **keine** gespeicherten Daten übertragen, und die Anfrage geht ausschließlich an den Server, dessen Seite der Nutzer ohnehin bewusst gemerkt hat. Abgeschwächt wird das durch:
  - `referrerPolicy="no-referrer"` – der Server erfährt nicht, von wo aus geladen wird, insbesondere nicht, dass die Anfrage aus einer Extension kommt
  - `loading="lazy"` – geladen wird nur, was tatsächlich sichtbar wird
  - Die Datenschutzerklärung muss diesen Punkt benennen; er ist der einzige, an dem überhaupt eine Verbindung nach außen entsteht
  - **Warum nicht das Bild selbst speichern:** `storage.local` fasst in Chrome/Edge 10 MB. Ein einziges eingebettetes Vorschaubild liegt schnell bei 100–500 KB, womit nach wenigen Dutzend Einträgen Schluss wäre. Sollte die Anfrage nach außen später unerwünscht sein, wäre der Weg eine eigene IndexedDB für die Bilddaten, nicht `storage.local`
