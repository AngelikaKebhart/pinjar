...
- **No third-party data transfer**: no data leaves the user's browser. Do not introduce any network call that sends stored data (or browsing data) to an external server. If a future feature (e.g., cloud sync) is proposed, it must be explicit opt-in and clearly communicated — never silently enabled.
  - The one existing outbound request is the Dashboard rendering preview images, which are stored as URLs rather than image data (`storage.local` holds 10 MB; embedded images would fill it after a few dozen entries). Opening the Dashboard therefore reveals the user's IP and timing to sites they saved. Keep the mitigations in place on any `<img>` showing saved content: `referrerPolicy="no-referrer"` and `loading="lazy"`. Do not add further remote resources — no web fonts, no icon CDNs, no favicon services — each one would be a new party learning what the user saved.
...
- **Minimal permissions principle**: prefer `activeTab` over broad host permissions like `<all_urls>`. Only add a permission when a concrete feature requires it, and note in the PR/commit why it's needed. The extension declares exactly `storage`, `activeTab`, `scripting` and `tabs` — **no host permission at all**, so it never gains access to the content of any page. Treat adding another one as a decision that needs explicit sign-off, not a convenience.
  - `tabs` was added deliberately for the badge and is the one exception to "as little as possible": a tab's address is unreadable without it, and `activeTab` only ever covers a tab the user just clicked the icon on — the very click the badge exists to spare them. It is the narrowest permission that delivers the feature; a host permission would additionally expose page content. The cost is real and was accepted: browsers show it as "read your browsing history" on install, so the extension no longer installs warning-free.
  - Tab addresses read for the badge are counted against locally saved domains and nothing else — never stored, never logged, never sent anywhere.
